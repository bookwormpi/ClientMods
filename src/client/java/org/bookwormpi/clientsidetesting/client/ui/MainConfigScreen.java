...
public class MainConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addDrawableChild(ButtonWidget.builder(
                Text.literal("Scan"),
                btn -> { BlockSearchFeature.enabled = true; BlockSearchFeature.lastPlayerChunk = null; if (MinecraftClient.getInstance().world != null && MinecraftClient.getInstance().player != null) { BlockSearchFeature.requestScan(MinecraftClient.getInstance(), MinecraftClient.getInstance().player.getChunkPos()); } }
        ).dimensions(rightX + indent, yRight, buttonWidth - indent, BUTTON_HEIGHT).build());
        yRight += BUTTON_HEIGHT + 2;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // Use vanilla background blur (no framebuffer stuff)
